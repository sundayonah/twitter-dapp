...
const MainPage = () => {
   const { getAllTweets, likeTweet } = useContext(TwitterDappContext);
         ...
         <main className="grid grid-cols-1 lg:grid-cols-1 gap-6 w-[80%] md:w-[75%] lg:w-[75%] m-auto my-10 border-solid border-2 border-gray-800">
            ...
            <div className="flex mt-2 flex flex-col">
               ...
               {getAllTweets.map((tweet) => (
                  <div
                     key={tweet.content}
                     className="w-full flex bg-transparent hover:bg-gray-500/20 p-2 border-b mb-2"
                  >
                     <div className="mx-2">
                        <img
                           ...
                           alt="logo-image"
                           className="h-6 w-10 rounded-full p-1 bg-gray-700"
                        />
                     </div>
                     <div className="">
                        ...
                        <div className="flex space-x-3 mt-2">
                           <button
                              onClick={() => likeTweet(tweet.id.toString())}
                           >
                              <span className="">
                                 <svg
                           ...
                           <p>{tweet.likes.toString()}</p>
                           {/* <p>ID: {tweet.id.toString()}</p> */}
                        </div>
                     </div>
                  </div>
               ))}
